...
values <- df[, c('chr', 'start', 'end', "Navarro2019.ATAC.H33KO", "Navarro2019.ATAC.H33KO.H33mut.resc", "Navarro2019.ATAC.H33KO.H33resc", "Navarro2019.ATAC.H33KO.H32resc")]

colnames(values) <- c('chr', 'start', 'end', "-", "+H33mut", "+H33", "+H32")
...
colnames(iap.df) <- c("-", "+H33mut", "+H33", "+H32")
...
iap.noout$variable <- factor(iap.noout$variable, levels=c('-','+H33','+H32','+H33mut'))
iap.melted.noout$variable <- factor(iap.melted.noout$variable, levels=c('-','+H33','+H32','+H33mut'))

p <- ggplot(df.noout, aes(x=variable, y=value)) +
  geom_violin(fill='#bbbbbb') +
  geom_jitter(data=iap.melted.noout, aes(x=variable, y=value, color=variable), alpha=0.7) +
  ylab('RPGC') +
  xlab('H33KO') +
