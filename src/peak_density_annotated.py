...
"""
This is a script for heatmap visualization and annotation ONLY. It works with
npz matrices generated by deepTools and annotations that must follow the same
order as these.

It allows to do some extra functions needed for larger panels, namely:
- Order heatmap rows taking as reference another matrix. These will be sorted by mean.
- Provide an arbitrary order to show the rows. This is useful for instance if
some kind of clustering was done externally.
- Override colorscale, axis limits.
- Set colorscales, titles and labels.
"""
...
class MData:
    ...
    def plot(self, axes, fig, cmap, interpolation, flank, body,
            ...
            loci_label='', title='', annot_title='', base_fontsize=12):
        ...
        heatmap_ax.set_ylabel('{} ({})'.format(loci_label, self.values.shape[0]), labelpad=10, fontsize=base_fontsize+4)
       
        # heatmap_ax.set_xlabel("RPGC", fontsize=12)
        heatmap_ax.set_xticklabels(['-{}kb'.format(flank/1000),
            '',
            '',
            '+{}kb'.format(flank/1000)], fontsize=base_fontsize-2)

        cbar_ax = axes[2, 0]
        fig.colorbar(heatmap, cax=cbar_ax, alpha=1, ticks=[0, hmax], orientation="horizontal")
        cbar_ax.tick_params(labelsize=base_fontsize-2)
        ...
        if override_mean_min is not None:
        ...
        # profile_ax.set_yticklabels(profile_ax.get_yticklabels(), fontsize=base_fontsize-2) 
        profile_ax.set_title(signal_label)
        profile_ax.tick_params(axis='both', which='major', labelsize=base_fontsize-2)
        ...
        if self.annotations:
            ...
            annotations_ax.set_xticklabels(current_annotations_labels, fontsize=base_fontsize, rotation='vertical')
            annotations_ax.set_title(annot_title, fontsize=base_fontsize+4)

        else:
            annotations_ax.axis('off')


def configure_plot(height, nannot, base_fontsize):
    ...
    heatmap_width = single_heatmap_width + 1
    ...
    figheight = height + cbar_height + average_plot_height + 3
...
if __name__ == '__main__':
    ...
    fig, ax = configure_plot(height=args.height, nannot=15, base_fontsize=28)
    ...
    mdata.plot(ax, fig, args.cmap, 'bilinear', args.flank, args.body,
        ...
        override_max=heatmap_max, override_mean_max=mean_max, override_mean_min=mean_min, base_fontsize=28)

    fig.suptitle(args.title)
    fig.savefig('{}.svg'.format(args.out), dpi=300)
